...
namespace UnityEngine.UI
{
    ...
    public class VertexHelper : IDisposable
    {
        ...
        public void CaluateUV1ByUV0()
        {
            if (m_Uv0S.Count != 4)
                return;
            Vector2 uvMin = m_Uv0S[0];
            Vector2 uvMax = m_Uv0S[2];
            Vector2 offset = uvMax - uvMin;
            for (int i = 0; i < m_Uv0S.Count; i++)
            {
                m_Uv1S[i] = (m_Uv0S[i] - uvMin) / offset;
            }
        }
        /// <summary>
        /// 2022/6/16 UV2的顶点数据修改为置灰使用
        /// </summary>
        /// <param name="_isGray"></param>
        public void CaluateUV1ByUV2(bool _isGray)
        {
            if (m_Uv2S.Count == 0)
            {
                return;
            ...
            var uv2 = new Vector2((_isGray ? 1 : 0), 0);
            for (int i = 0; i < m_Uv2S.Count; i++)
            {
                m_Uv2S[i] = uv2;
            }
        }
