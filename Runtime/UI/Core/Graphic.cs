...
namespace UnityEngine.UI
{
          ...
          ICanvasElement
    {
        ...
        /// </summary>
        public static Material s_ABDefaultUI = null;

#if OPTIMIZE_UGUI
        // HuaHua.
        static List<Canvas> cacheCanvasList = new List<Canvas>();
#endif
        ...
        static public Material defaultGraphicMaterial
        {
            ...
            {
                //StrawberryYang
                if (s_ABDefaultUI != null)
                {
                    return s_ABDefaultUI;
                }
                if (s_DefaultUI == null)
...
#if OPTIMIZE_UGUI
        //HuaHua.
        [NonSerialized] private bool m_LayoutDirty;
#endif
        ...
        public Canvas canvas
        {
            get
            {
                // HuaHua.
#if OPTIMIZE_UGUI
                if ((System.Object)m_Canvas == null)
                    CacheCanvas();
                return m_Canvas;
#else
                if (m_Canvas == null)
...
#if OPTIMIZE_UGUI
            //HuaHua.
            gameObject.GetComponentsInParent(false, cacheCanvasList);
            var count = cacheCanvasList.Count;
            if (count > 0)
            {
                // Find the first active and enabled canvas.
                for (int i = 0; i < count; ++i)
                {
                    if (cacheCanvasList[i].isActiveAndEnabled)
                    {
                        m_Canvas = cacheCanvasList[i];
                        break;
                    }
                }
            }
