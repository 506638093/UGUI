...
namespace UnityEngine.UI
{
    ...
    public abstract class MaskableGraphic : Graphic, IClippable, IMaskable, IMaterialModifier
    {
        ...
        //反向遮罩 可以用于抠图
        [SerializeField]
        private bool m_ReverseMask = false;
        ...
        public virtual Material GetModifiedMaterial(Material baseMaterial)
        {
            ...
            {
                //如果反向遮罩 CompareFunction取反
                var maskMat = StencilMaterial.Add(toUse, (1 << m_StencilValue) - 1, StencilOp.Keep, m_ReverseMask? CompareFunction.NotEqual:CompareFunction.Equal, ColorWriteMask.All, (1 << m_StencilValue) - 1, 0);
        ...
        /// 获取原始贴图的大小
        /// </summary>
        public virtual Vector2 GetNativeSize()
        {
            return Vector2.zero;
        }
        ...
        private Rect rootCanvasRect
        {
            get
            {
                ...
                for (int i = 1; i < 4; i++)
                {
                    var cx = m_Corners[i].x;
                    var cy = m_Corners[i].y;
                    min.x = cx < min.x ? cx : min.x;
                    min.y = cy < min.y ? cy : min.y;
                    max.x = cx > max.x ? cx : max.x;
                    max.y = cy > max.y ? cy : max.y;
                }
