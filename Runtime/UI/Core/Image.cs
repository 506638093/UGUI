    ...
    public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        public override void SetNativeSize()
        {
            if (activeSprite != null)
            {
                ...
                rectTransform.sizeDelta = GetNativeSize();
                SetAllDirty();
            }
        }
        ...
        protected override void OnPopulateMesh(VertexHelper toFill)
        {
            ...
            switch (type)
            {
                ...
                case Type.Sliced:
                    ...
                    break;
                case Type.Tiled:
                    //GenerateTiledSprite(toFill); 优化平铺 不生成顶点使用拉伸九宫格 shader中处理
                    GenerateSlicedSprite(toFill);
                    break;
                case Type.Filled:
            ...
            toFill.CaluateUV1ByUV0();
            toFill.CaluateUV1ByUV2(base.isGray);
        }
        ...
        void GenerateSimpleSprite(VertexHelper vh, bool lPreserveAspect)
        {
            ...
            var color32 = color;
            vh.Clear();
            vh.AddVert(new Vector3(v.x, v.y), color32, new Vector2(uv.x, uv.y), uv1, uv1, uv1, normal, tangent);
            vh.AddVert(new Vector3(v.x, v.w), color32, new Vector2(uv.x, uv.w), uv1, uv1, uv1, normal, tangent);
            vh.AddVert(new Vector3(v.z, v.w), color32, new Vector2(uv.z, uv.w), uv1, uv1, uv1, normal, tangent);
            vh.AddVert(new Vector3(v.z, v.y), color32, new Vector2(uv.z, uv.y), uv1, uv1, uv1, normal, tangent);
        ...
        static void AddQuad(VertexHelper vertexHelper, Vector2 posMin, Vector2 posMax, Color32 color, Vector2 uvMin, Vector2 uvMax)
        {
            ...
            vertexHelper.AddVert(new Vector3(posMin.x, posMin.y, 0), color, new Vector2(uvMin.x, uvMin.y), uv, uv, uv, normal, tangent);
            vertexHelper.AddVert(new Vector3(posMin.x, posMax.y, 0), color, new Vector2(uvMin.x, uvMax.y), uv, uv, uv, normal, tangent);
            vertexHelper.AddVert(new Vector3(posMax.x, posMax.y, 0), color, new Vector2(uvMax.x, uvMax.y), uv, uv, uv, normal, tangent);
            vertexHelper.AddVert(new Vector3(posMax.x, posMin.y, 0), color, new Vector2(uvMax.x, uvMin.y), uv, uv, uv, normal, tangent);
